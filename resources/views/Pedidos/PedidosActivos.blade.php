  ...
  <body>
    ...
    <form action="{{ route('buscar') }}" method="POST">
      ...
      <div class="display">
       <div class="col-md-11">
        ...
        <input type="hidden" value="PedidosAct" id="validationCustomUsername" aria-describedby="inputGroupPrepend" method="POST" name="Tabla" accept="image/*"required>
    ...
    <table class="table table-striped">
      ...
      <tr>
        ...
        <td>{{ $Servicio->nombrePaso }}</td>
    
        <td>
          <form method="GET" action="/Pedidos/Activos/{{$Servicio->id}}/edit">
            @csrf
          <div class="col-12">
        ...
        <td><a href="/Pedidos/Activos/{{ $Servicio->id }}">Ver</a></td> 
      </tr>
      @endforeach
    </tbody>
      </table>
      ...
      <div id="pagination-links" class="pagination">
        ...
        <div>
            @if ($ServiciosPaginados->currentPage() > 1)
                <a href="/Pedidos/Activos?page={{ $ServiciosPaginados->currentPage() - 1 }}" class="btn btn-primary" style="margin-left: 7px">Anterior</a>
            @endif
    
            @if ($ServiciosPaginados->hasMorePages())
                <a href="/Pedidos/Activos?page={{ $ServiciosPaginados->currentPage() + 1 }}" class="btn btn-primary" style="margin-left: 7px">Siguiente</a>
            @endif
        </div>
    </div>
