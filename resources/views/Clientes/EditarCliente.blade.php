...
<head>
  ...
  <body>
    ...
    <form class="row g-3 needs-validation" method="POST" action="/Clientes/{{$Cliente->id}}">
      @csrf
      @method("PUT")
  
      <div class="col-md-6">
          <label for="validationCustom01" class="form-label">Nombre</label>
          <input type="text" class="form-control @error('Nombre') is-invalid @enderror" id="validationCustom01" value="{{ old('Nombre', $Cliente->Nombre) }}" name="Nombre" required>
          @error('Nombre')
      ...
      <div class="col-md-6">
          ...
          <div class="input-group has-validation">
              <input type="text" class="form-control @error('Telefono') is-invalid @enderror" id="validationCustomUsername" value="{{ old('Telefono', $Cliente->Telefono) }}" name="Telefono" aria-describedby="inputGroupPrepend" required>
              @error('Telefono')
                  <div class="invalid-feedback">
                      {{ $message }}
                  </div>
      ...
      <div class="col-md-6">
          ...
          <input type="email" class="form-control @error('Correo') is-invalid @enderror" id="validationCustom02" value="{{ old('Correo', $Cliente->Correo) }}" name="Correo" required>
          @error('Correo')
              <div class="invalid-feedback">
                  {{ $message }}
              </div>
      ...
      <div class="col-md-6">
        ...
        <select class="form-control @error('Tipo') is-invalid @enderror" id="Tipo" name="Tipo" required>
            <option value="Fisica" {{ old('Tipo', $Cliente->Tipo) == 'Fisica' ? 'selected' : '' }}>Física</option>
            <option value="Moral" {{ old('Tipo', $Cliente->Tipo) == 'Moral' ? 'selected' : '' }}>Moral</option>
        </select>
        @error('Tipo')
            <div class="invalid-feedback">
                {{ $message }}
